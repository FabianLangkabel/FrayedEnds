#This a file to test the whole workflow of the spinorbital refinement with the outer dmrg loop, currently for H3lin sto-3g
...
def get_coefficients_and_integrals(mol, init_guess, iter, folderpath):
    uhf=scf.UHF(mol)
    if init_guess is None:
        uhf.kernel()
    if init_guess is not None:
        uhf.kernel(init_guess)
    ...
    if iter > 0:
        h1alpha = np.load(f"{folderpath}/alpha_hmatrix.npy")
        h1beta = np.load(f"{folderpath}/beta_hmatrix.npy")
        h1e = (h1alpha, h1beta)
        g2_alphaalpha = np.load(f"{folderpath}/alpha_alpha_gtensor.npy")
        g2_alphabeta = np.load(f"{folderpath}/alpha_beta_gtensor.npy")
        g2_betabeta = np.load(f"{folderpath}/beta_beta_gtensor.npy")
        g2e = (g2_alphaalpha, g2_alphabeta, g2_betabeta)
...
#use this to make the geometry file for H3 linear
def make_geometry_file_H3lin(directory, position):
    x = position.replace(".", "_")
    filename = f"{directory}/{x}_geometry_H3lin.mol"
    with open(filename, "w") as file:
        l1 = "geometry \n"
        l2 = "units Bohr \n"
        l3 = "no_orient 1 \n"
        l4 = "eprec 1e-6 \n"
        l5 = "H 0.00000000 0.00000000 0.00000000 \n"
        l6 = f"H {position} 0.00000000 0.00000000 \n"
        l7 = f"H -{position} 0.00000000 0.00000000 \n"
        l8 = "end"
...
def make_orbitaltranslator_json(directory, pos):
    ...
    dict = {
        ...
        "molecule_file": f"{directory}/{pos}_geometry_H3lin.mol",
        "calpha_coeff_file": alpha_path,
        "cbeta_coeff_file": beta_path,
        "output_folder": directory,
    }
    dict_json = json.dumps(dict, indent=4)
    output_jsonfile = f"{directory}/{pos}_H3lin_OrbitalTranslator.json"
...
def make_spinorbopt_json(directory, pos, iter, num_orbs): 
    #pos = pos.replace(".", "")
    dict_1 = {
        "box_size": 50.0,
        "wavelet_order": 8,
    ...
    output_jsonfile = f"{directory}/{pos}_H3lin_SpinorbOpt.json"
...
#testpath = f"/workspaces/MRA-OrbitalOptimization/automate_process/H3_test"
#os.mkdir(testpath)

#set up only for one specific geometry

H_pos_H3lin_num = str(1.8) #specify H position, I can change this in the loop
H_pos_H3lin_str = H_pos_H3lin_num.replace(".", "_")

outputfolder = f"/workspaces/MRA-OrbitalOptimization/H3lin_test" #specify output_dir
#os.mkdir(outputfolder) #make output_dir
H3lin_geom = define_H3lin_mole_object(H_pos_H3lin_num, 'sto-3g') #make mole object
make_geometry_file_H3lin(outputfolder, H_pos_H3lin_num) #make geometry file
orbtrans_file = make_orbitaltranslator_json(outputfolder, H_pos_H3lin_str) #make orbitaltranslator json file
spin_squared = []
total_energy = []
for i in range(0, 4, 1):
    spin_squared.append(i)
    total_energy.append(i)
    delete_tmp_file('/workspaces/MRA-OrbitalOptimization/automate_process/tmp4')
    H3lin_alpha_coeffs, H3lin_beta_coeffs, ncas, n_elec, spin, ecore, h1e, g2e, orb_sym = get_coefficients_and_integrals(H3lin_geom, init_guess=None, iter=i, folderpath=outputfolder) #run UHf and get the coefficients
    if i == 0:
        save_mocoeffs(H3lin_alpha_coeffs, H3lin_beta_coeffs, outputfolder, H_pos_H3lin_str)
    ...
    save_rdms(alpha_1rdm, beta_1rdm, aa_2rdm, ab_2rdm, bb_2rdm, outputfolder, H_pos_H3lin_str)#save the rdms in npy files
    spinorbopt_file = make_spinorbopt_json(outputfolder, H_pos_H3lin_str, i, 3) #make spinorbopt json file
    ...
    try: 
        ...
        for line in run_spinorbopt.stdout:
            print(line, end='')
            if "S squared" in line:
                spin_squared.append(line.strip())
            if "Total energy" in line:
                refined_energy = line.strip()
        run_spinorbopt.wait()
    except subprocess.CalledProcessError as e:
        print("Error occurred:")
        print(e.stderr)
    total_energy.append(refined_energy)

with open(f"{outputfolder}/final_energies.txt", 'w') as file1:
    for i in total_energy:
        file1.write(f"{i}\n")
with open(f"{outputfolder}/spin_squared.txt", 'w') as file2:
    for j in spin_squared:
        file2.write(f"{j}\n")
