...
def test_methods_from_pno():
    ...
    for method in ["spa", "hcb-upccgd", "upccgsd", "fci"]:
        ...
        geom = geom.lower()
        energy, orbitals, rdm1, rdm2 = madpy.optimize_basis_3D(world=world, many_body_method=method, geometry=geom, econv=1.e-3, orbitals=orbitals)
...
def test_methods_from_minbas():
    ...
    for method in ["upccgsd", "fci"]:
        ...
        geom, test_energy = data
        geom = geom.lower()
        energy, orbitals, rdm1, rdm2 = madpy.optimize_basis_3D(world=world, many_body_method=method, geometry=geom, econv=1.e-3, orbitals=orbitals, **kwargs)
        assert numpy.isclose(energy, test_energy, atol=1.e-3)
    del world
