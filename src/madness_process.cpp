...
template <std::size_t NDIM>
void MadnessProcess<NDIM>::plot(std::string filename, Function<double, NDIM> f, std::string axis, int datapoints) {
    ...
    plot_line(filename.c_str(), datapoints, lo, hi, f);
}

template <std::size_t NDIM>
void MadnessProcess<NDIM>::plane_plot(std::string filename, Function<double, NDIM> f, std::string plane, double zoom,
    ...
    std::vector<Function<double, NDIM>> MRAf = {f};
    plot_plane<NDIM>(*world, MRAf, filename, params);
}

template <>
void MadnessProcess<3>::cube_plot(std::string filename, Function<double, 3> f, MolecularGeometry molecule, double zoom,
                                  int datapoints, std::vector<double> origin) {
    PlotParameters params;
    ...
    plot_cubefile<3>(*world, f, outfile, molecule.mol.cubefile_header(params.origin<3>()), params.npoints(),
