...
iterations = 10 # max number of iteration
econv = 1.0e-6  # energy convergence threshold

# ---------------------------------------------------------------------------
# .dat Dateien fuer Resultate
# ---------------------------------------------------------------------------
with open("iterations_benzene_oo.dat", "w") as f:
    header = "iteration iteration_time_s energy_0"
    f.write(header + "\n")

with open("results_benzene_oo.dat", "w") as f:
    header = "energy_0"
    f.write(header + "\n")
...
# Orbitale speichern
def save_orbitals(core_orbitals, active_orbitals, base_path, iteration, converged):
    iter_path = os.path.join(base_path, f"it_{iteration:02d}")
    os.makedirs(iter_path, exist_ok=True)
    core_count = 0
    active_count = 0

    for orb in core_orbitals:
        orb.save_to_file(os.path.join(iter_path, f"core_{core_count}.fe"))
        core_count += 1

    for orb in active_orbitals:
        orb.save_to_file(os.path.join(iter_path, f"active_{active_count}.fe"))
        active_count += 1

    print(f"Saved {core_count} core + {active_count} active orbitals to '{iter_path}/'")
...
H_eff = integrals.compute_effective_hamiltonian(core_orbitals=core_orbs, active_orbitals=active_orbs, V=Vnuc, energy_offset=nuclear_repulsion_energy, g_ordering='chem')
c = H_eff[0]
h1 = H_eff[1]
G_act = H_eff[2]
...
with open("iterations_benzene_oo.dat", "a") as f:
    f.write(f"{-1} {0.00} {rdm_energy:.15f}\n")
...
for iteration in range(iterations):
    ...
    core_orbs, active_orbs, converged = opti.refine_orbitals(
    ...
    save_orbitals(core_orbs, active_orbs, output_orb_path, iteration, converged)
    ...
    driver = DMRGDriver(scratch="./tmp", symm_type=SymmetryTypes.SU2, n_threads=DMRG_THREADS, stack_mem=DMRG_STACK_MEM)
    driver.initialize_system(n_sites=n_active_orbitals, n_elec=n_active_electrons, spin=0)
    ...
    with open("iterations_benzene_oo.dat", "a") as f:
        f.write(f"{iter} {iter_time:.2f} {rdm_energy:.15f}\n")

    if np.isclose(rdm_energy, current, atol=econv, rtol=0.0):
        print(f"Energy converged after iteration {iteration}")
        break  # Loop terminates as soon as the energy changes less than econv in one iteration step
    current = rdm_energy

with open("results_benzene_oo.dat", "a") as f:
    f.write(f"{rdm_energy:.15f}\n")
