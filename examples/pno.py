...
def solve(world, geometry, n_act, n_core, name, tt=1e-6, clo=0.001, ceps=1e-6, Blo=0.001, Beps=1e-6):
    ...
    for iteration in range(5):
        ...
        fci_start = time()
        # FCI calculation
        e, fcivec = fci.direct_spin1.kernel(h1, g2, n_act, n_act_electrons)  # Computes the energy and the FCI vector
        ...
        opti_start = time()
        opti = fe.OrbitalRefinement(
            world, Vnuc, nuc_repulsion, truncation_tol=tt, coulomb_eps=ceps, coulomb_lo=clo, BSH_lo=Blo, BSH_eps=Beps
        )
        if iteration == 0:
