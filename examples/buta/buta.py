...
from spa import SPASolver, make_decomposed_clusters, run_spa
...
def init_buta():
    mol = hb.Molecule(
        geometry=geometry, basis_set="sto-3g", backend="pyscf"
    ).use_native_orbitals()
    tqmol = tq.Molecule(
        geometry=geometry, basis_set="sto-3g", backend="pyscf"
    ).use_native_orbitals()
...
def solve_spa(mol, U):
    ...
    result = vqe_solver(
        circuit=U, molecule=mol, H=mol.make_hardcore_boson_hamiltonian()
    )
    print(result.energy)
    rdm1, rdm2 = vqe_solver.compute_rdms(
        U, variables=result.variables, molecule=mol, use_hcb=True
    )
...
mol = tq.Molecule(
    geometry=geometry, one_body_integrals=h, two_body_integrals=g, nuclear_repulsion=c
)
print(mol)
