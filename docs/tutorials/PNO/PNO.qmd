...
In the following we will see how to compute pair-natural orbitals (PNOs) at the basis set limit,
using the implementation originally described in ()[].
We will see how we can use these PNOs to construct second-quantized Hamiltonians that can then be
used to compute energies with classical and variational quantum methods.

Note that we will not use MP2-F12 (see [here]()) but plain MP2 to determine the PNOs.


## Relevant Literature
- [Original description]() of directly determined PNOs.
- [Application]() in the context of this tutorial.
- [Diagonal Approximation]() used as a default in `FrayedEnds`

# Directly Determined PNOs in a Nutshell

The usual procedure for using pair-natural orbitals is
1. Solve Hartree-Fock in the given basis set (Roothan Hall)
2. Solve MP2 in the given basis set (using the virtual orbitals from the Roothan Hall procedure)
...
1. Solve HartreeFock at the basis set limit (see [](); only occupied orbitals)
...
We refer to []() for further details and just sketch the general structure.
...
5. Refine PNOs (similar fixed-point equation as Hartree-Fock above, see for the explicit potentials []())
...
where we used the notation from []() to indicate method/basis(number of electrons, number of spin orbitals).
...
For more qualitative and quantitative benchmarks we refer to []() and [](). 
